...
async def startup(ctx):
  ...
  for role in ctx.guild.roles:
    if role.name.startswith(cModRolePrefix):
      ...
      continue
    await info(ctx, guilds[ctx.guild.id], 'Found: `@{}`'.format(role.name))
  ...
  if bool(guilds[ctx.guild.id].roleAmongUs) == False:
    await info(ctx, guilds[ctx.guild.id], 'Creating `@{}`'.format(kAmongUsRoleName))
  ...
  for channel in ctx.guild.channels:
    if channel.name == kBotChannelName:
      ...
      continue
    await info(ctx, guilds[ctx.guild.id], 'Found: `#{}`'.format(channel.name))
  ...
  if bool(guilds[ctx.guild.id].channelLog) == False:
    await info(ctx, guilds[ctx.guild.id], 'Creating bot log: `#{}`'.format(kLogChannelName))
    overwrites = {
...
class NotkBotGuild:
  ...
  async def notifyAmongUsGame(self, ctx, channel, code):
    ...
    code = code.upper()
    await info(ctx, self, "Notifying `@{}` of Among Us game code `{}` in `#{}`".format(self.roleAmongUs.name, code, channel.name))
    await channel.send(\
